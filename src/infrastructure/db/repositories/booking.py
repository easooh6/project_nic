...
class BookingRepository:
    ...
    async def create_booking(self, entity: Booking) -> Booking | None:
        
        async with self.session_factory() as session:

            query_check = select(BookingModel).where(BookingModel.user_id==entity.user_id,
                                                     BookingModel.created_at==entity.created_at
                                                     )
            ...
            return entity.model_validate(booking_model)
    ...
    async def update_booking_status(self, booking: Booking) -> int:

        async with self.session_factory() as session:

            query = update(BookingModel).where(BookingModel.id == booking.id).values(
